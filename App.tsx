...
import { INITIAL_TRIP_DATA, KEY_TIPS, CORE_CONCLUSION } from './constants';
import { TripDay, Activity, Tip } from './types';
import { 
      ...
      <main className="flex-grow max-w-lg mx-auto w-full px-5 pb-24 mt-[-60px] relative z-20">
        ...
        <div className="space-y-14">
          {trip.map((day) => (
            <div 
              key={day.id} 
              ref={(el) => el && observerRefs.current.set(day.id, el)}
              ...
              <div className="flex items-end justify-between mb-8 px-2">
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <div className="w-14 h-14 rounded-3xl ocean-gradient rotate-3 flex flex-col items-center justify-center text-white font-bold shadow-lg shadow-sky-200">
        ...
        <section className="mt-32 text-center pb-20 px-4">
          ...
          <button 
            ...
            className="w-full py-5 ocean-gradient text-white rounded-[2rem] font-black text-lg shadow-xl shadow-sky-400/30 flex items-center justify-center gap-3 active:scale-[0.97] transition-all"
          >
            獲取旅程祝福 <Info size={22} />
          </button>
        </section>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-6 bg-sky-950/60 backdrop-blur-xl transition-opacity">
          <div className="bg-white w-full max-w-sm rounded-[3.5rem] shadow-2xl overflow-hidden p-12 text-center relative border-[6px] border-sky-50">
            ...
            <button 
              ...
              className="w-full py-5 ocean-gradient text-white rounded-[2rem] font-black text-lg shadow-lg active:scale-95 transition-all"
            >
              開啟夢幻假期
            </button>
          </div>
